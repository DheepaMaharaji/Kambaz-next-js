...
export default function ManageFoldersScreen() {
  ...
  const loadFolders = async () => {
    try {
      setLoading(true);
      const data = await client.getFoldersForCourse(courseId);
      setFolders(data);
    } catch (error) {
  ...
  const handleAddFolder = async () => {
    ...
    try {
      const newFolder = await client.createFolder(courseId, newFolderName.trim());
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      await client.deleteFolders(courseId, Array.from(selectedFolders));
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const updatedFolder = await client.updateFolder(editingId, editingName.trim());
